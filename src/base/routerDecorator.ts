...
function route(method: Methods, path: string, middlewares: Middleware[] = []) {
  return function(target: any, key: string) {
    ...
    const controller = controllerRegistry.find(c => c.name === className);
...
export async function RegisterRoutes() {
  ...
  controllerRegistry.forEach(controller => {
    ...
    const controllerMiddleWares: any[] = Reflect.getMetadata("middlewares", instance) || [];

    controller.routes.forEach(route => {
      const { method, routeFunctionName, middlewares } = route;
      const handler = newinstance[routeFunctionName].bind(newinstance);

      const fullPath = (controllerPath + route.path).replace("//", "/");
...
async function loadControllers() {
  ...
  const isDev = file.endsWith(".ts");

  for (const module of modules) {
    const controllerPath = path.join(modulesPath, module, `Controller${isDev ? ".ts" : ".js"}`);
